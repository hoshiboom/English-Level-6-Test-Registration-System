...
public class LoginCheckInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (!request.getMethod().equals("OPTIONS")) {
            ...
            boolean signupCondition = method.equals("POST") && (url.contains("teacher") || url.contains("student") || url.contains("admin"));
            //2.判断url是login则放行
            if (url.contains("login/admin") || url.contains("login/teacher") || url.contains("login/student") || signupCondition) {//有漏洞：被恶意构造?login=...
                log.info("请求跳过拦截，url=" +request.getRequestURL().toString());
                return true;
            }
            ...
            //        }
            //</editor-fold>
            log.info("请求未跳过拦截，url=" +request.getRequestURL().toString());
            //3.获取请求头的令牌
